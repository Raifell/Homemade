# Задание №1
# а) Загрузите массив json - объектов с сайта https://jsonplaceholder.typicode.com/todos
# б) Сохраните массив в отдельные файлы, в отдельные папки
...
def form(name, todos):
    ...
    for x in range(len(count_file)):
        if not os.path.isdir(f"folder-{x}"):
            os.mkdir(f"folder-{x}")
        with open(f"folder-{x}/{name[x]}", 'w') as file:

...
class Example:
    def __init__(self):
        self.win = tk.Tk()
        self.win.title('Task_3(B-2)')
        self.win.geometry('350x300+200+300')
        self.win.resizable(False, False)

        self.labelA = tk.Label(self.win, text='Программа выводит разницу двух дат \n(в днях, часах, минутах и секундах)', bg='yellow')
        self.labelA.grid(row=0, column=0, ipady=5, sticky='we', columnspan=2)
        self.labelB = tk.Label(self.win, justify=tk.LEFT)
    ...
    def get_date(self):
        try:
            ...
            result = a1 - b1
            self.labelB['text'] = f'{abs(result.days)} день\n' \
                                  f'{abs(result.days) * 24} часов\n' \
                                  f'{abs(result.days) * 1440} минут\n' \
                                  f'{abs(result.days) * 86400} секунд'
        except:
